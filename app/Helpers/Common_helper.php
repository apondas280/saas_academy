...
if (! function_exists('course_by_instructor')) {
    ...
    {
        if ($course_id != '') {
            $user_id      = DB::table('courses')->where('id', $course_id)->first();
            $byInstructor = DB::table('users')->where('id', $user_id->user_id)->first();
            return $byInstructor;
        }
    }
}
...
if (! function_exists('get_enroll_info')) {
    function get_enroll_info($course_id = "", $user_id = "")
    {
        if ($course_id != '' && $user_id != "") {
            $enrolled = DB::table('enrollments')->where('course_id', $course_id)->where('user_id', $user_id)->first();
            return $enrolled;
        }
    }
}
    ...
    function get_course_creator_id($course_id = "")
    {
        if ($course_id != '') {
            ...
            foreach ($course as $value) {
                $creator = DB::table('users')->where('id', $value->user_id)->first();
            }
            return $creator;
        }
    }
...
if (! function_exists('category_course_count')) {
    ...
    {
        if ($slug != '') {
            $slug_category = DB::table('categories')->where('slug', $slug)->first();
...
if (! function_exists('is_root_admin')) {
    function is_root_admin($admin_id = '')
    {
        ...
        $root_admin_id = DB::table('users')->limit(1)->orderBy('id', 'asc')->first()->id;
...
if (! function_exists('has_permission')) {
    function has_permission($route = '', $user_id = '')
    {
        ...
        $root_admin_id = DB::table('users')->first()->id;
...
if (! function_exists('lesson_progress')) {
    function lesson_progress($lesson_id = "", $user_id = "", $course_id = "")
    {
        ...
        if ($query->count() > 0) {
            $lesson_ids = json_decode($query->first('completed_lesson'), true);
...
if (! function_exists('is_permission')) {
    function is_permission($permission_for = '', $admin_id = '')
    {
        ...
        if (is_root_admin($admin_id)) {
            ...
            $get_admin_permissions = $get_admin_permissions->first();
...
if (! function_exists('instructor_experience')) {
    ...
    {
        $first_course = App\Models\Course::where('user_id', $user_id)->first();

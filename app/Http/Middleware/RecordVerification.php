...
class RecordVerification
{
    ...
    public function handle(Request $request, Closure $next, $table, $param1, $param2 = null): Response
    {
        if (! Schema::hasTable($table)) {
            return redirect()->route(auth()->user()->role . '.team.packages')->with('error', get_phrase('Data table not found.'));
        }
        ...
        if (! $query->exists()) {
            return redirect()->route(auth()->user()->role . '.team.packages')->with('error', get_phrase('Data not found.'));
        }
        return $next($request);
    }
}

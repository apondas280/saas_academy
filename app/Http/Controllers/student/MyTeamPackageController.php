...
class MyTeamPackageController extends Controller
{
    public function index()
    {
        $page_data['packages'] = TeamPackagePurchase::join('team_training_packages', 'team_package_purchases.package_id', 'team_training_packages.id')
            ...
            ->latest('id')->paginate(6)->appends(request()->query());
    ...
    public function search_members(Request $request, $company = "", $package_id = '')
    {
        $user = User::where('email', $request->email)->select('users.id', 'users.name', 'users.email', 'users.photo')->first();
        if ($package_id && $user) {
            $status = TeamPackageMember::where('leader_id', auth()->user()->id)->where('team_package_id', $package_id)->where('member_id', $user->id)->exists() ? 1 : 0;
    ...
    public function member_action($company = "", $action)
    {
        ...
        if ($action == 'register') {
            ...
            if (! $package_member) {
                ...
                $enroll['entry_date']      = date('');
                if ($package->expiry_type == 'limited') {
                    $enroll['expiry_date'] = date('Y-m-d H:i:s', $package->expiry_date);
                }

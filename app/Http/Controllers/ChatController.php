...
class ChatController extends Controller
{
    ...
    public function chat($reciver, $product = null)
    {
        ...
        $messageThrade = MessageThread::where(function ($query) use ($reciver, $user_id) {
        ...
        if (! empty($messageThrade)) {
            Chat::where('MessageThread', $messageThrade->id)->where('reciver_id', $reciver)->where('read_status', '0')->update(['read_status' => '1']);
            $message = Chat::where('MessageThread', $messageThrade->id)->orderBy('id', 'DESC')->limit('20')->get();
        } else {
            $message = [];
        }
    ...
    public function chat_save(Request $request)
    {
        ...
        $firstmessageThrade = MessageThread::where(function ($query) use ($reciver, $user_id) {
            $query->where('sender_id', $reciver)
                ->where('reciver_id', $user_id);
        })->orWhere(function ($query) use ($reciver, $user_id) {
            $query->where('sender_id', $user_id)
        ...
        $messageThradeCount = MessageThread::where(function ($query) use ($reciver, $user_id) {
            $query->where('sender_id', $reciver)
                ->where('reciver_id', $user_id);
        })->orWhere(function ($query) use ($reciver, $user_id) {
            $query->where('sender_id', $user_id)
        ...
        dd($messageThradeCount);

        if ($messageThradeCount <= 0) {
            $messageThrade             = new MessageThread();
            ...
            if ($done) {
                ...
                $chat->MessageThread = $messageThrade->id;
                ...
                if (is_array($request->multiple_files) && $request->multiple_files[0] != null) {
                    ...
                    foreach ($request->multiple_files as $key => $media_file) {
                        ...
                        if ($file_extention == 'avi' || $file_extention == 'mp4' || $file_extention == 'webm' || $file_extention == 'mov' || $file_extention == 'wmv' || $file_extention == 'mkv') {
                            ...
                            FileUploader::upload($media_file, 'message/images');
                            $file_type = 'image';
                        }
                ...
                $page_data['message'] = Chat::where('MessageThread', $messageThrade->id)->orderBy('id', 'DESC')->limit('1')->get();
            ...
            $chat->chatcenter    = $request->messagecenter;
            $chat->message       = $request->message;
            $chat->MessageThread = $firstmessageThrade->id;
            ...
            $page_data['message'] = Chat::where('MessageThread', $firstmessageThrade->id)->orderBy('id', 'DESC')->limit('1')->get();
            return view('frontend.my-courses.message', $page_data);
        }
    }
}

...
{{-- <form action="{{ route('quiz.submit_m', $quiz->id) }}" method="post" class="quiz-submit-form">@csrf --}}
    <form action="{{ route('quiz.submit.mobile', ['id' => $quiz->id]) }}" method="post" class="quiz-submit-form">
        @csrf
        <input type="hidden" name="quiz_id" value="{{ $quiz->id }}">
        {{-- <input type="text" name="user_id" value="{{ $user_ids }}"> --}}
        {{-- <input type="text" name="user_id" value="{{ isset($_GET['user_id']) ? $_GET['user_id'] : '' }}"> --}}
                    ...
                    @if ($question->type == 'mcq')
                        ...
                        @foreach ($options as $index => $option)
                            <div class="col-sm-6">
                                ...
                                <label class="form-check-label text-capitalize" for="{{ $option }}-{{ $question->id }}">{{ $option }}</label>
                            </div>
                        @endforeach
...
@if ($questions->count() > 0)
    <div class="row">
        <div class="col-12 d-flex gap-3 justify-content-center">
            <button type="button" class="continue-leason-btn w-auto" id="prevBtn" onclick="prevQuestion()"><i class="fi fi-rr-angle-small-left"></i>{{ get_phrase('Prev') }}</button>
            <button type="button" class="continue-leason-btn w-auto" id="nextBtn" onclick="nextQuestion()">{{ get_phrase('Next') }}<i class="fi fi-rr-angle-small-right"></i></button>
            @if ($submits->count() < $quiz->retake)
                <button type="button" class="continue-leason-btn w-auto active d-none" id="submitBtn" onclick="submitQuiz()">{{ get_phrase('Submit') }}<i class="fi fi-rr-badge-check ms-2"></i></button>
            @endif
        </div>
    </div>
@endif

<div class="w-100">
    ...
    <div class="mb-3 d-flex gap-2 align-items-center flex-wrap">
        ...
        @if ($sections->count() > 0)
            <a href="#" onclick="ajaxModal('{{ route('modal', ['instructor.course.lesson_type', 'id' => $course_details->id]) }}', '{{ get_phrase('Add new lesson') }}')" class="btn ol-btn-light ol-btn-sm">{{ get_phrase('Add lesson') }}
            </a>
        @endif

        <a href="#" onclick="ajaxModal('{{ route('modal', ['instructor.course.quiz_create', 'id' => $course_details->id]) }}', '{{ get_phrase('Add new quiz') }}')" class="btn ol-btn-light ol-btn-sm d-none">{{ get_phrase('Add quiz') }}</a>

        @if ($sections->count() > 0)
            <a href="#" onclick="ajaxModal('{{ route('modal', ['instructor.course.section_sort', 'id' => $course_details->id]) }}', '{{ get_phrase('Sort sections') }}')" class="btn ol-btn-light ol-btn-sm">{{ get_phrase('Sort Section') }}
            </a>
        @endif
    </div>

<div class="w-100">
    ...
    <div class="mb-3 d-flex gap-2 align-items-center flex-wrap">
        <a href="#" onclick="ajaxModal('{{ route('modal', ['admin.course.create_section', 'id' => $course_details->id]) }}', '{{ get_phrase('Add new section') }}')"
            class="btn ol-btn-light ol-btn-sm">{{ get_phrase('Add section') }}
        </a>

        @if ($sections->count() > 0)
            <a href="#" onclick="ajaxModal('{{ route('modal', ['admin.course.lesson_type', 'id' => $course_details->id]) }}', '{{ get_phrase('Add new lesson') }}')"
                class="btn ol-btn-light ol-btn-sm">{{ get_phrase('Add lesson') }}
            </a>
        @endif

        <a href="#" onclick="ajaxModal('{{ route('modal', ['admin.course.quiz_create', 'id' => $course_details->id]) }}', '{{ get_phrase('Add new quiz') }}')"
            class="btn ol-btn-light ol-btn-sm d-none">{{ get_phrase('Add quiz') }}</a>

        @if ($sections->count() > 0)
            <a href="#" onclick="ajaxModal('{{ route('modal', ['admin.course.section_sort', 'id' => $course_details->id]) }}', '{{ get_phrase('Sort sections') }}')"
                class="btn ol-btn-light ol-btn-sm">{{ get_phrase('Sort Section') }}
            </a>
        @endif
    </div>
                ...
                <div class="accordion-btn-wrap">
                    ...
                    <div class="accordion-button-buttons">
                        ...
                        <a href="#" data-bs-toggle="tooltip" title="{{ get_phrase('Delete section') }}"
                            onclick="confirmModal('{{ route('admin.section.delete', $section->id) }}'); event.stopPropagation();" class="delete">
                                ...
                                <li>
                                    ...
                                    <div class="buttons">
                                        ...
                                        <a href="#" data-bs-toggle="tooltip" title="{{ get_phrase('Delete lesson') }}"
                                            onclick="confirmModal('{{ route('admin.lesson.delete', $lesson->id) }}')" class="edit-delete">
                                            <span class="fi-rr-trash"></span>
                                        </a>
                                    </div>
                                </li>

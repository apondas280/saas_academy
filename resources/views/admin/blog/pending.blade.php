            ...
            <div class="row p-3">
                ...
                <div class="col-md-8">
                    <div class="d-sm-flex mb-3 mb-sm-0 justify-content-md-end gap-3">
                        <form action="{{ route('admin.blog.pending') }}" method="get" class="flex-grow-1 flex-md-grow-0">
                            <div class="search-input flex-grow-1 h-100">
                                ...
                                @if (isset($_GET) && count($_GET) > 0)
                                    <a href="{{ route('admin.blog.pending') }}" class="clear-search" data-bs-toggle="tooltip" title="{{ get_phrase('Clear') }}"><i class="fi-rr-cross-circle"></i></a>
                                @endif
                            </div>
                        </form>
                                ...
                                <tbody>
                                    @foreach ($blogs as $key => $blog)
                                        <tr>
                                            ...
                                            <td>
                                                <div class="dAdmin_info_name min-w-150px">
                                                    <span class="badge rounded-pill text-bg-{{ $blog->status ? 'success' : 'danger' }}">{{ get_phrase($blog->status ? 'Active' : 'Inactive') }}</span>
                                                </div>
                                            </td>

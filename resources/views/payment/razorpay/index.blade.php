@php
    ...
    if ($model == 'InstructorPayment') {
        ...
        if ($public_key == '' || $secret_key == '') {
            $msg = 'This payment gateway is not configured.';
        }
    } elseif ($model == 'PurchaseCourse' || $model == 'BootcampPurchase') {
...
@else
    ...
    <div class="alert alert-danger d-flex align-items-center" role="alert">
        ...
        <div class="payment_err_msg">
            <p class="text-start">
                <span class="fw-700">{{ get_phrase('Opps!') }}</span>
                <span>{{ get_phrase($msg) }}</span>
                <span class="d-block ps-5">{{ get_phrase('Try another gateway.') }}</span>
            </p>
        </div>
    </div>
@endif
